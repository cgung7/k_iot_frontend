...
class Library {
  ...
  returnBook(id) {
    ...
    if(book) {
      ...
      console.log('해당하는 찾을 찾을 수 없습니다.');
    }

  }

  updateBook(id, newTitle, newAuthor) {
    // +) find 메서드는 배열에서 제공된 콜백함수를 만족하는 첫 번쨰 요소를 반환
  ...
  filterBooksByTitle(keyword) {
    ...
    const filtered = this.books.filter(book => book.title.toLowerCase().iscludes(keyword.toLowerCase()));
  ...
  filterBooksByAvailable(isAvailable) {
    // +) isAvailavle 값에 따라 true면 출력 시 ${대여 가능}인 책 목폭
  ...
  // [통계] 대여 가능 도서수 집계
  countAvailableBooks() {
    // isAvailable이 true인 데이터만 추출해서 해당 배열의 길이를 측정
...
busanLibrary.addBook('자바스크립트 공부는 재밌어', '최광섭');
...
busanLibrary.updateBook(2, 'Java studty is fun', 'lee di kyeong');
...
yangsanLibrary.addBook('자바스크립트 공부는 재밌어', '최광섭');
yangsanLibrary.addBook('자바 공부는 재밌어', '이도경');
yangsanLibrary.addBook('SQLD 공부는 재밌어', '이승아');
