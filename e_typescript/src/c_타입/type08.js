...
someValue = true; // 어떤 값이든 할당 가능하지만 개발자가 문자열로 단언하고 싶을 경우
...
//    >> 각 DOM 객체가 가지는 고유한 속성에 접근하기 위해서는 반드시 "타입 단언"이 필요하다
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // TS 컴파일러는 실시간 코드 변환은 하지 않음
    // : 코드 수정 시 재컴파일러 실행 필요하다.
});
//& 2)JSON 문자열
// - 객체(parse) <> JSON(stringfy)
// : JSON 문자열을 분석(parse)하여 반환되는 데이터
//   any타입으로 간주
var jsonString = '{"name": "CGS", "age": 35}';
