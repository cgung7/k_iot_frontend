...
//! === 푸드트럭 예약 시스템 === //
// : 푸드트럭 예약 관리 화면에서, 관ㄹ리자가 고객의 예약 상태를 실시건으로 모니터링하고 승인
/*
  @ 1) 트럭 예약 데이터 불러오기 
    - API 호출
    - 마운트 시 실행
  ...
  @ 2) 일정 주기 자동 새로고침
    ...
    ? 폴링(Polling)
      : 클라이언트가 서버에게 주기적으로 요청을 보내 
          새로운 데이터나 상태 변화를 확인하는 방식

  @ 3) 예약 상태 변경
    - 버튼 클릭 시 수정 요청 후 UI 갱싱
    ...
    try {
      ...
      const mappedData: Reservation[] = data
        ...
        .map((item: any, idx: number) => ({
          ...
          time_slot: `12:${idx}0 PM`, // 시간대 예시 (12시 부터 10분씩 차이)
