...
//? JWT 기반 인증 구조
// : 일반적인 웹앱 로그인 과정
// 1. 사용자 아이디/비밀번호로 로그인 요청을 보냄
// 2. 서버는 검증 후 Access Token + Refresh Token 두 개 발급
// 3. 클라이언트는 Access Token을 저장(보통 메모리/Zustand 등)
//  +) Refresh Token은 HTTP Only 쿠키로 저장
// 4. 이후 토큰이 필요한 API 요청 시 Acthorization: Bearer<AccessToken> 형태로 보냄
...
//? AccessToken이 짧을 경우의 불편한 점
// : AccessToken의 만료기간(10 ~ 15분) 마다 다시 로그인
// - 해당 불편함을 줄이기 위해 Refresh Token이 존재한다.
