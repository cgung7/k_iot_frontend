...
//  : HTML5 History API를 사용하여 브라우저 주소(URL)와 리액치 UI를 동기화
//  - 앱루트 최상단 main.tsx 또는 App.tsx에 반드시 명시해야 함
//    하위 컴포넌트에서 React Router DOM 사용 가능
//% - 새로고침 없이 URL만 변경 (SPA -Single Page Application)
//% - 라우터의 최상위 컴테이너로 한 번만 감싸면 됨

//& <Routes></Routes>
//  : 여러 <Route />들을 묶어서 관리
//  - <BrowserRouter></BrowserRouter> 내부에서 사용 가능
//% - 여러 개의 <Route />를 감싸서 "URL"과 일치하는 Route만 렌더링
//% - 한 번에 하나의 Route만 렌더링 (매칭된 첫 번쨰 Route)
//% - 자식으로 반드시 <Route />만 가질 수 있음

// cf) React Router v6에서 <Swith> -> <Routes>로 변경
...
//    children: 중첩 라우드(NEsted Routes) 정의 가능
