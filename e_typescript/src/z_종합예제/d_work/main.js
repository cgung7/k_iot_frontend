...
var TaskLogger = /** @class */ (function () {
    ...
    TaskLogger.prototype.deleteTask = function (taskId) {
        // 현재의 tasks 배열을 순회되는 요소의 id와 삭제하고자 하는 요소의 id(매개변수)가 일치하지 않는 요소만 새로운 배열에 담아 tasks에 저장
        this.tasks = this.tasks.filter(function (task) { return task.id !== taskId; });
        this.renderTasks();
    };
    TaskLogger.prototype.createTaskElement = function (task) {
        var taksItem = document.createElement("div");
        taksItem.className = "task-item";
    ...
    TaskLogger.prototype.renderTasks = function () {
        ...
        if (taskList) {
            ...
            this.tasks.forEach(function (task) {
                var taksItem = _this.createTaskElement(task);
...
var init = function () {
    ...
    window.addEventListener('click', function (e) {
        // 이벤트가 발생한 요소 범위가 modal 내부의 modal-content가 아니라
        // , 실질적인 taskModal 인 경우
