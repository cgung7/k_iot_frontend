...
//& 2. 비동기 작업을 사용하여JSONPlaceholder에서 사용자 데이터 가져오는 함수 정의
async function fetchUserData(userId: number) {
  try {
    ...
    const data = await response.json(); // 객체 현식으로 데이터 반환
    ...
    if (data.id) {
      const userData: FetchResponse = {
        ...
        company: {
          ...
          catchPhrace: data.company.catchPhrace,
        },
      };
