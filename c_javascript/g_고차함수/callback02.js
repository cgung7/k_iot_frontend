...
// - index: 순화되는 각 요소의 인덱스 번호
// - array: 배열 그 자체

// >> 콜백함수의 인자값은 필수X (주로 value만 사용)
//      : 사용하지 않는 인자값은 생략 가능

//? function callback(value) {}
//? function callback(value, index) {}
//? function callback(, index, array) {} - 순서가 중요 (비워지는 인자값의 공간은 남겨둬야한다)
//? function callback(,, array) {}

//! '배열'의 콜백 함수 메서드
// : 각 요소에 대해 동일한 함수 실행
// - 배열 내부의 요소를 매개변수로 사용하는 콜백 함수로 호출
...
//    '콜백 함수의 리턴값이 true인 요소만 모여서 새로운 배열을 생성하여 반환'
console.log('=== filter() ===');

const basicArray = [0, 1, 2, 3, 4, 5];

const evenNumbers1 = basicArray.filter(v => v % 2 === 0);
...
tasktags.forEach(tasktags => console.log(tasktags));
...
cars.filter(function(car) {
  ...
  return car.length > 4;
  // 함수 실행 후 true를 반환한 요소 만을 새로운 배열에 담에 반환
});
