...
function passingScores(scores) {
  ...
  // 3) 60점 이상인 점수들의 평균: 합의 단일한 값으로 반환 - reduce
  const passingAverage = passing.reduce((acc, scores) => acc + scores, 0) / passingCount;
...
//? 함수 호출
const result = passingScores(scores); // [ 7, 59.57142857142857 ]
console.log(result);

// SyntaxError: Identifier 'passingCountValue' has already been declared
// const passingCountValue = result[0];
// const passingAverageValue = result[1];


//! cf) 구조 분해 할당
// : 배열이나 객체읙 각 요소를 반 번에 각 변수에 할당
...
const filteredWords = (words, substring) => {
  // 1) 첫 번째 인자: 배열
  // 2) 두 번쨰 인자: 필터링할 단어
  // >> words(배열)의 각 단어 중 substring(문자열)이 포함된 단어만 필터링하여 변환

  // +) includes() 메서드 사용: 문자열.includes(문자)
