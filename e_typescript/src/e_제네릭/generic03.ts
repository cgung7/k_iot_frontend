...
UpdateUser(1, {name: '이승아'});
console.log(users[1]); // { name: '이승아', age: 35 }
UpdateUser(2, {name: '김지선'});
console.log(users[2]); // { name: '김지선', age: 20 }

UpdateUser(1, {age:25});
UpdateUser(2, {age:15});
console.log(users[1], users[2]); // { name: '이승아', age: 25 } { name: '김지선', age: 15 }

UpdateUser(1, {});
console.log(users[1]); // { name: '이승아', age: 25 }
...
// user.name = '김세훈';
// user.age = 30;
// : 객체 자체가 변수일지라도 내부 속성들이 readonly 상수처럼 취굽
...
// >> Omit의 두 번째 인자는 유니온 타입(or, |) 사용 가능

const salatyA: EmployeeForAccount = {
  id: 1,
  name: '최광섭',
  age: 35,
...
const salatyB: EmployeeForGeneral = {
  id: 1,
  name: '최광섭',
  age: 35,
