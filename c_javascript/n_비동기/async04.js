...
//! async, await 특징
// : 비동기적 코드를 동기적으로 작성 (기독성 향상)
// - async 함수 내에서만 await 키워드 사용 가능
// - 하나의 async 함수 내에서 여러 개의 await 사용가능

//! 비동기 작업 예시 (+ 예외 처리)
// : 외부와 연결되는 작업을 주로 '비동기 작업'으로 작성
// +) 예외 처리 코드와 함께 사용 '권장'
...
async function fetchCustomerData() {
  try {
      ...
      // .json(): json 데이터를 parse분석하여 JS 객체로 변환
    ...
    console.log(error); // TypeError: Failed to parse URL from url
  }
...
// +) 성공 시 resolve 같이 Response 객체
// +) 실패 시 reject 값에 예외가 담김 - 네트워크 자체가 실패했을 때만 발생 (404)
