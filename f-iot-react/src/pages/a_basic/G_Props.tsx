...
/* 
  cf) 컴포넌트(Component): 리액트의 구정 요소(UI)

  ! Props(Properties): 컴포넌트의 속성
  : Props로 데이터 전달
    <ChildComponent name="최광섭" />
...
function ChildComponent(props: { name: string }) {
  // props.name로 사용
  // : props는 readonly 속성이기 떄문에 값의 재할당이 불가
  // props.name = '최광섭';
...
function G_Props() {
  ...
  return (
    ...
    <MultiProps name='홍길동' colorProps='lightblue' />
    <MultiProps2 name='김삿갓' colorProps='coral' />
    <MultiProps2 colorProps='green' />

    {/* ...props가 구조 분해 할당을 사용하여 각각의 속성명을 찾아간다. */}
    <MultiProps2 {...props} />
