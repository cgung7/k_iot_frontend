...
export const api: AxiosInstance = axios.create({
  baseURL: API_BASE,
  // 10초 타임아웃 - 요청이 10초 이상 걸릴 경우 요청 취소 + 타임아웃 에러 발생
  timeout: 10000,
  headers: {
...
//? Axios 특징
// 1) Promise 기반: .then() / async | await 사용 가능
// 2) 자동 JSON 처리: 객체를 JSON으로 변환하거나 파싱 자동
// 3) 응답 객체 구조화: response.data만 확인
// 4) 인더셉터 지원: 요청/응답 가로채기 가능 (토큰/에러 처리 등)
