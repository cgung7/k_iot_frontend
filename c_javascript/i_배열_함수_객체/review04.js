...
! 2) GradeManagement 클래스
  ...
  - 학생별 평균 성적 계산: getAverageScore()
    > 모든 학생의 id, 이름, 평균 성적을 포함하는 새 배열을 반환
    > map(), reduce()
  - 조건에 따른 학생 필터링 & 정렬
    > getTopStudents(threshold)
      : 평균 성적이 주어진 값(한계점) 이상인 학생을 필터링 + 내름차순 정렬 반환
    > filter(), sort()
...
class Student {
  ...
  getAverageScore() {
    ...
    //? cf) 객체내부 구조는 하나의 쌍으로 (키-key: 값-value)구성
...
class GradeManagement {
  ...
  getAverageScore() {
    //? 자바스크립트에서는 {}는 함수 본문으로 인식
    // : 객체 리터럴 반환 시 JS에게 해당 문법 구조가 코드 블록이 아닌 객체임을 전달하기 위해
    //    ()소괄호 사용
    //    >> {}: 코드 블록
    //    >> ({}): 객체 리터럴
    //    화살표함수에서 객체를 즉시 반환할 때는 ()로 감싸야 한다. 
...
// 학생 추가 예제
gradeSystem.addStudnet("최광섭", { Math: 90, English: 85, Sciense: 78});
