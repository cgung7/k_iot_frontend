...
function State01() {
  ...
  // 명명 규칙: set + 상태명 (name -> setName, const -> setConst, post -> setPost)
  ...
  const handleUpClick = () => {
    // setConst(5); // 상태 설정 함수의 인자값이 상태 변수로 전달
    // setConst(count + 1); // 0 + 1 === 1
    // setConst(count + 1); // 0 + 1 === 1
    ...
    //      setCount(() => {})
    //      >> 해당 콜백 함수의 인자는 '상태의 최산값'
    //      >> prev-상태명 (prevName, prevCount, prevUser / previous 이전의)
    setConst(prevCount => prevCount + 1); // 0 + 1 === 1
    setConst(prevCount => prevCount + 1); // 1 + 1 === 2
  }
